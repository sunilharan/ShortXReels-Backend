...
export const validateCreateReel = expressAsyncHandler(async (req: any, res, next) => {
  try {
    ...
    for (const id of categories) {
      const exists = await Category.exists({ _id: new mongoose.Types.ObjectId(id as string) });
      if (!exists) {
        res.status(404);
    ...
    const videoFiles = mediaFiles.filter((f: any) => f.mimetype.startsWith('video/'));
    const imageFiles = mediaFiles.filter((f: any) => f.mimetype.startsWith('image/'));
